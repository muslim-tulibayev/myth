...
<div
    x-data="review({{ $cards->toJson() }}, '{{ route('collections.review.store', [$collection, '__ID__']) }}')"
...
>
    ...
    <template x-if="mode === null">
        <div class="bg-white rounded-xl border border-gray-200 px-8 py-10 text-center">
            ...
            <h2 class="text-xl font-semibold text-gray-900 mb-1">Ready to review?</h2>
            <p class="text-sm text-gray-500 mb-8" x-text="cards.length + ' card' + (cards.length !== 1 ? 's' : '') + ' due'"></p>
    ...
    <template x-if="done">
        <div class="bg-white rounded-xl border border-gray-200 px-8 py-10 text-center">
            ...
            <h2 class="text-xl font-semibold text-gray-900 mb-2">Session complete</h2>
...
<script>
function review(cards, routeTemplate) {
    return {
        mode: null,
        cards: cards,
        index: 0,
        ...
        async answer(isCorrect) {
            ...
            const url = routeTemplate.replace('__ID__', this.cards[this.index].id);
            await fetch(url, {
                method: 'POST',
                headers: {
