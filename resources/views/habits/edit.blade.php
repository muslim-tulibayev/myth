...
<form method="POST" action="{{ route('habits.update', $habit) }}"
      x-data="{ quantified: {{ $habit->isQuantified() ? 'true' : 'false' }} }">
    @csrf
    @method('PUT')
    ...
    <div class="space-y-5">
        ...
        <div>
            ...
            <input type="text" name="name" value="{{ old('name', $habit->name) }}" required autofocus
        ...
        <div>
            ...
            <textarea name="goal" rows="3" required
                      class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-gray-400">{{ old('goal', $habit->goal) }}</textarea>
        ...
        <div x-show="quantified" x-cloak class="flex gap-3">
            <div class="flex-1">
                ...
                <input type="number" name="target" value="{{ old('target', $habit->target) }}" min="0.01" step="any"
                       class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-gray-400">
                @error('target') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
            </div>
            <div class="flex-1">
                <label class="block text-sm font-medium mb-1">Unit</label>
                <input type="text" name="unit" value="{{ old('unit', $habit->unit) }}" placeholder="glasses, km, pages…"
                       class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-gray-400">
        ...
        <div class="flex gap-3">
            <div class="flex-1">
                ...
                <input type="text" name="emoji" value="{{ old('emoji', $habit->emoji) }}" placeholder="🧘"
                       class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-gray-400">
